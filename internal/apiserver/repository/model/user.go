...
type UserM struct {
	...
	Email     sql.NullString `gorm:"column:email;type:varchar(100);unique;not null;comment:用户电子邮箱地址" json:"email"` // 用户电子邮箱地址
	Phone     sql.NullString `gorm:"unique"`
	CreatedAt time.Time      `gorm:"column:createdAt;type:datetime(3);not null;default:CURRENT_TIMESTAMP(3);comment:用户创建时间" json:"createdAt"`                                  // 用户创建时间
	UpdatedAt time.Time      `gorm:"column:updatedAt;type:datetime(3);not null;default:CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3);comment:用户最后修改时间" json:"updatedAt"` // 用户最后修改时间
}
